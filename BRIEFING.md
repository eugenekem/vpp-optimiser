...
- **v31 — the BM leg no longer decides with perfect foresight; the shadow log's BM/total figures are now honestly labelled and (going forward) realistic on this axis.** Closes the gap v30 found. **Design chosen by a bake-off, not assumed:** scratch test over 200 days (2026-03-05..09-20), BM leg only, 30% capacity, cost-aware, settled at the real imbalance price, comparing what BM could *legitimately* decide on — perfect foresight (ceiling) £43.9k/day; **real DA price £36.9k (84.1% of ceiling, no losing days)**; real DA + 7-day mean imbalance-minus-DA spread £35.7k (81.3%); 50/50 blend £33.8k (77.0%); 7-day mean of imbalance prices £27.2k (62.1%); yesterday's imbalance prices £21.6k (49.3%, 6 losing days). The simplest option won and the fancier ones did worse. It is also legitimate: DA has cleared before BM matters, the same reason ID may know the DA price.
  ...
  **Not adopted, and the reason is about what the gain is made of, not its size.** After v31, the ID and BM legs decide knowing the real DA price while the DA leg trades on a forecast, so moving capacity out of DA plausibly harvests the DA forecast error (`reg_demand` captures ~87% of the DA-shape ceiling; ID/BM effectively capture 100% of it) rather than a real ID/BM edge — an untested hypothesis, not a measured cause. And the ID price is still synthetic (real DA + Normal(0, £5)) and BM fill/acceptance is unmodelled, so the recommended 80% ID+BM split would concentrate the book in exactly the legs whose modelling is least realistic — raising the share of P&L that is not investor-safe. `config.py` defaults are unchanged. **Revisit when ID has a realistic price model or BM acceptance is modelled**; a decisive test would re-run this sweep with ID/BM deciding on a DA *forecast* (no hindsight) — if the edge vanishes, it was forecast-error harvesting. Only the 4-hour class was swept; Battery_1 (2-hour) was not.
