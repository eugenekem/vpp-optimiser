...
**Version:** 22.0
**Status:** Phase 1 replay and Phase 2 shadow trading built. DA price forecasting uses Elexon's **day-ahead wind/solar + demand forecast**: +24.5% accuracy skill, **86.2% cost-aware P&L capture (£26,807/day central, £24,702/day conservative)**, edge widening as costs rise — see section 10d. Forecasts still NOT wired into dispatch. **v22: an unattended daily pipeline now exists** (`scripts/daily_pipeline.py`) — built, tested, and run for real, closing a 40-day data gap and an 88-day shadow-logging backlog in one run (section 17). Cloud scheduling of it is the immediate next step. **Repo relocated to `~/repos/vpp-optimiser_ClaudeCode`** (was `~/Documents/...`, symlinked for compatibility) after iCloud sync was found actively interfering with git — see section 17.
...
| Cloud scheduling of the daily pipeline | ⬜ Next |
