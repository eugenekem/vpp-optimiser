...
**Version:** 23.0
**Status:** Phase 1 replay and Phase 2 shadow trading built. DA price forecasting uses Elexon's **day-ahead wind/solar + demand forecast**: +24.5% accuracy skill, **86.2% cost-aware P&L capture (£26,807/day central, £24,702/day conservative)**, edge widening as costs rise — see section 10d. Forecasts still NOT wired into dispatch. **v23: the daily pipeline is now genuinely live and unattended** — `scripts/daily_pipeline.py` runs automatically every day at 05:00 UTC via a cloud routine (independent of the maintainer's laptop being on), reached after 4 real test iterations found and fixed 3 cloud-specific bugs (no `gh` CLI, detached-HEAD checkouts, missing deps — see section 17). Clock-change crash in `dispatcher.py` also fixed and verified this version. **Repo relocated to `~/repos/vpp-optimiser_ClaudeCode`** (was `~/Documents/...`, symlinked for compatibility) after iCloud sync was found actively interfering with git — see section 17.
