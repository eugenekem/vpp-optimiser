...
- **v29 — ID and BM closed the cost-awareness gap v28 explicitly left open; the live path has no remaining cost-blind leg.** `optimise_battery_id`/`optimise_battery_bm` gained the same `cost_discharge`/`cost_charge` parameters as `optimise_battery_lp` (default `0.0`, so any other caller is unaffected), applied inside each LP objective the same way. `dispatcher.py` passes the **same** cost figures already computed for the DA leg into both — degradation, exchange fees, and market impact are properties of trading the physical battery, not of which market layer the trade happens in, so reusing one set of constants across all three legs is a deliberate simplification (documented here, same spirit as the DA cost model). Settlement gained matching cost-adjusted columns (`id_price_discharge`/`id_price_charge`, `ssp_settle`/`sbp_settle`), and `shadow.py` now settles ID/BM on these instead of raw price × power. BM's bespoke inline revenue/cost calc was also folded into the shared `gross_rev_cost` helper `dispatcher.py` already used for DA/ID, removing duplication.
  ...
  **Also noticed while investigating this, unrelated and out of scope:** the same pipeline run also produced an exploration note (`data/explorations/2026-09-17/`) comparing the first live cost-aware dispatch day against price volatility — worth a look separately, not reviewed as part of this fix.
