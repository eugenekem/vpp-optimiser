...
**Three-arm pilot, 58 real days (16 Jul – 14 Sep 2026), cost-aware, α=0.90, λ=0.25** (chosen from a 20-day sweep across λ ∈ [0, 0.25, 0.5, 1.0] — see below):
...
**Finding 1 — a better central estimate did NOT help.** `ensemble_mean − deterministic` = **−£444/day**. Averaging several forecast methods together performed slightly *worse* than trusting `reg_demand`'s own point forecast alone on this sample — a real, if modest, negative result, not the "more inputs, better outcome" instinct one might expect.

**Finding 2 — the hedge genuinely improved the worst days, at a real cost.** `cvar_stochastic − ensemble_mean` = **−£193/day** on average, but the worst-10%-of-days average (p5) rose from £24,010 to £25,765 (**+£1,755/day**) — CVaR is doing exactly what it's designed to do, trading some average return for tail protection. The single worst day barely moved either way (£20,536 → £21,241 → £21,216), suggesting whatever makes the single worst day bad isn't something a DA-only hedge can fix.

**Honest bottom line: mixed, not a clean win, on this sample.** The insurance premium (average-day cost, −£637/day vs the plain deterministic arm) is larger in raw pounds than the payout (tail improvement, +£1,755/day only shows up on ~10% of days). The 20-day λ-sweep that preceded this pilot showed the same non-monotonic pattern — λ=0.25 improved the tail over λ=0, but λ=0.5 and λ=1.0 did *not* keep improving it while costing progressively more on average — so this is not simply "turn the dial further for more protection." **What this does NOT prove:** that CVaR hedging never helps — 58 days is a pilot, not a verdict (this project has been burned by small-sample conclusions before, e.g. the v16 `weekday` illusion). It does NOT test ID/BM recourse, only the DA leg. It has NOT been wired into `shadow.py`'s default, and should not be until a larger sample and/or a different λ/α shows a cleaner result.

**What did NOT change:** `optimiser_lp.py::optimise_battery_lp` — zero edits. `forecast.py` — zero edits, `forecast_residuals.py` only calls its existing public functions. `replay.py` — zero edits, always calls `run_dispatcher(date)` with the default `None`. `dispatcher.py`'s behaviour for `da_forecast_method=None` or any non-`_stochastic` method name — verified byte-identical. `shadow.py`'s current default (`reg_demand`, non-stochastic) — deliberately left untouched; switching it is a separate, explicit decision gated on stronger evidence than this pilot provides.
...
| Stochastic optimisation — hedge across a price distribution | ✅ Built + piloted (v26) — mixed result, see section 17 |
...
- **Price forecasting remains the blocker for real trading — now started, not solved.** Phase 1 (replay.py) and Phase 2 (shadow.py) both decide using published/already-known prices. `forecast.py` (v15) established measured baselines, but accuracy is not yet tradeable (see section 10b). Open questions from here:
  ...
  - **✅ ADDRESSED (v26) — CVaR-hedged stochastic DA optimisation.** See section 10e for the full result. Genuine, proven hedging mechanism (not a disguised point-forecast, per the mean-equivalence proof), built on empirical historical forecast-error scenarios rather than an assumed noise shape — but the 58-day pilot result is mixed (real tail-risk improvement, at an average-day cost that outweighed it in raw pounds on this sample), so not yet adopted anywhere. Larger-sample and λ/α-sensitivity work is the natural follow-up, not a re-design.
