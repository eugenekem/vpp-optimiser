...
**Version:** 32.0
**Status:** Phase 1 replay and Phase 2 shadow trading built. Daily pipeline split across GitHub Actions (fetch, 05:00 UTC) and a cloud routine (check-in, alert, exploration sense-check, 10:30 UTC). **v24: the long-standing settlement-date misalignment bug (open since v15) is fixed** — `fetch_da_prices.py`/`fetch_wind_solar.py` corrected, all ~730 days of historical data migrated and verified lossless. **v25: all headline numbers re-validated against the corrected data — confirmed negligible impact**, and the forecast is wired into `shadow.py`'s actual daily dispatch for the first time. **v26/v27: CVaR-hedged stochastic DA optimisation built, then re-tested at full scale — a 58-day pilot looked promising, but the full ~718-day history shows it does not pay for itself at the tested settings; not adopted as the default.** **v28: found and fixed a real gap — the proven cost-aware algorithm (v20) was built and validated but never actually wired into the live dispatch/shadow path, which had been running cost-blind for its entire history.** **v29: closed the ID/BM half of that gap v28 explicitly left open — the whole DA→ID→BM live path is now cost-aware end to end, for the first time.** **v30: found that the BM leg decides with perfect foresight of the real imbalance price — ~29% of the logged shadow P&L — so the logged total is NOT investor-safe; only the DA-only forecast-driven figures (section 10d) are.** A capacity-split sweep that appeared to show +12% from shifting capacity to BM was traced to this artifact and not adopted — see section 17. **v31: BM now decides on the real DA price (settled at the real imbalance price) instead of the future — worth ~9% of the logged total; the pre-v31 rows are labelled `bm_basis=foresight`, not rewritten.**
...
## 10e. CVaR-hedged DA-only stochastic optimisation

**Scripts:** `models/forecast_residuals.py`, `models/optimiser_lp_stochastic.py`, `models/da_stochastic_pnl.py` **Output:** `data/forecast_residuals_reg_demand.csv`, `data/da_stochastic_pnl.csv`

Section 10d's DA leg trades on `reg_demand`'s single point forecast, then settles at the real price — but trusts that one predicted path completely. This asks: instead of committing to one guess, can the DA schedule be hedged across a *distribution* of plausible price outcomes?

**The load-bearing check, done before any code was trusted:** a naive "maximise expected revenue across N scenarios" stochastic LP is mathematically **identical** to solving the plain LP once on the scenario mean price — because the dispatch decisions are shared across all scenarios (no recourse) and the objective/constraints are linear in price with no scenario-dependent term, linearity of expectation collapses it to the mean. Proven algebraically, then confirmed numerically on real data by `optimiser_lp_stochastic.py`'s own `__main__` smoke test: λ=0 CVaR objective matched the plain LP on the scenario mean to **£0.0000** difference, and N=1 collapsed CVaR to that scenario's own revenue exactly. **Without a risk-aversion term, "stochastic optimisation" would just be a disguised, weaker version of "use a better point forecast."** CVaR (Conditional Value at Risk, Rockafellar–Uryasev linear formulation) is the standard fix — LP-representable, no new solver needed.

**Scenarios are built from real data, not an assumed noise shape.** `forecast_residuals.py` reconstructed `reg_demand`'s actual historical forecast error, period by period, across 722 days (34,648 individual residuals) — nothing in the codebase had ever stored this before, only aggregate MAE. Spot-check: mean absolute residual **£17.73/MWh** vs `reg_demand`'s independently-measured MAE of **£17.72/MWh** in `forecast_accuracy.csv` — confirms the extraction is correct. Scenarios are generated by bootstrapping *whole historical days'* real error patterns onto today's forecast (preserving realistic within-day correlation, unlike `simulate_intraday_prices`'s iid Normal noise) — with one guardrail added after a real finding: 2025-01-08's genuine £1,353/MWh cold-snap price spike (the all-time high in the dataset) produced a −£1,178/MWh residual that, when transplanted onto an unrelated calm day, produced an unrealistic −£991/MWh scenario. Clipped to [−£200, £1,500] (wider than the real observed [−£103, £1,353] range, to preserve genuine tail risk while ruling out combination artifacts that never actually happened).

**Wired into `dispatcher.py` via a `"_stochastic"` suffix** (e.g. `da_forecast_method="reg_demand_stochastic"`), only engaging if the base forecast succeeded with no fallback — residual history is method-specific, so hedging a forecast that isn't the one requested would be inconsistent. Verified on 2026-09-14: default (`None`) and `"reg_demand"` both reproduced their already-known figures exactly (£69,465.36 and £67,234.37); `"reg_demand_stochastic"` ran cleanly with `da_basis="reg_demand_stochastic"` and per-battery CVaR diagnostics, and **ID/BM schedule files were byte-identical (same SHA-256) before and after** — confirming ID/BM are genuinely untouched, exactly as designed.

**Pilot, 58 real days (16 Jul – 14 Sep 2026), cost-aware, α=0.90, λ=0.25** (chosen from a 20-day sweep across λ ∈ [0, 0.25, 0.5, 1.0]):

| Arm | Mean/day | Median/day | p5/day (worst ~10%) | Worst day | Capture |
|---|---|---|---|---|---|
| deterministic (`reg_demand` point forecast) | £43,355 | £36,813 | £24,641 | £20,536 | 88.4% |
| ensemble_mean (mean of scenarios) | £42,911 | £35,958 | £24,010 | £21,241 | 87.5% |
| cvar_stochastic (λ=0.25 hedge) | £42,718 | £34,349 | £25,765 | £21,216 | 87.1% |

On 58 days, the hedge looked genuinely promising: tail (p5) up £1,755/day vs the ensemble mean, for an average-day cost of only £193/day. **This pilot number should NOT be trusted on its own** — this project has been burned by exactly this shape of small-sample illusion before (the v16 `weekday` forecast method looked +15.8% better on 30 days, then collapsed to worse-than-`mean_7` on 730). It was explicitly flagged as a pilot, not a verdict, and re-run at scale below.

**Full-history re-run, 718 real days (all available, 04 Sep 2024 – 15 Sep 2026), same α=0.90, λ=0.25 — the trustworthy number:**

| Arm | Mean/day | Median/day | p5/day (worst ~10%) | Worst day | Capture |
|---|---|---|---|---|---|
| deterministic (`reg_demand` point forecast) | £27,760 | £24,176 | £14,129 | −£16,188 | 86.6% |
| ensemble_mean (mean of scenarios) | £27,321 | £23,749 | £13,334 | −£13,449 | 85.2% |
| cvar_stochastic (λ=0.25 hedge) | £27,015 | £23,769 | £13,800 | −£14,180 | 84.3% |

**The 58-day tail improvement was small-sample noise. At full scale it does not hold up.** `cvar_stochastic − ensemble_mean` = **−£306/day** on average (worse than the 58-day sample's −£193/day), and the tail (p5) improvement shrinks from the pilot's +£1,755/day to just **+£466/day** — nowhere near enough to cover the average-day cost. Worse: the single worst day, which the 58-day pilot suggested was roughly unaffected either way, is actually **£731 more negative** under the hedge than under the plain ensemble mean (−£13,449 → −£14,180) — the opposite of what a hedge is supposed to do. Capture declines monotonically across all three arms (86.6% → 85.2% → 84.3%), consistent with cost accumulating at every step with no offsetting benefit.

**Conclusion, revised from "mixed" to a real answer: at λ=0.25/α=0.90, this specific CVaR hedge does not pay for itself, and the full-scale data is the reason to believe that, not the 58-day pilot that looked encouraging.** The mean-equivalence proof, the empirical residual-based scenarios, and the wiring into `dispatcher.py` are all still sound and verified — what full-scale testing settled is the *parameter choice and payoff*, not the mechanism. **What this does NOT prove:** that no λ/α setting could ever work (only this one combination was tested at scale; a broader sweep at full scale is the natural follow-up if this is revisited), and it does NOT test ID/BM recourse, only the DA leg. It has NOT been wired into `shadow.py`'s default, and given this result, should not be.

**What did NOT change:** `optimiser_lp.py::optimise_battery_lp` — zero edits. `forecast.py` — zero edits, `forecast_residuals.py` only calls its existing public functions. `replay.py` — zero edits, always calls `run_dispatcher(date)` with the default `None`. `dispatcher.py`'s behaviour for `da_forecast_method=None` or any non-`_stochastic` method name — verified byte-identical. `shadow.py`'s current default (`reg_demand`, non-stochastic) — deliberately left untouched.

**Confirmed reproducible, not just re-run:** the full-history run reused the same per-date deterministic scenario seeding as the pilot — the original 58 days' figures in `data/da_stochastic_pnl.csv` came back byte-identical when embedded in the larger 718-day run, rather than drifting, confirming that fix from the 20-day sweep holds under a different run context too.
...
| Wire forecast into dispatch (shadow.py's DA leg now built on reg_demand) | ✅ Done (v25) — see section 17 |
| Stochastic optimisation — hedge across a price distribution | ✅ Built + tested at full scale (v26/v27) — does not pay for itself at tested settings, see section 17 |
| Wire proven cost-awareness into the live dispatch/shadow path (DA, then ID/BM) | ✅ Done (v28 DA, v29 ID/BM) — the live path has no remaining cost-blind leg, see section 17 |
| AI agent layer | ⬜ To do |
| Settlement reconciliation | ⬜ To do |
| Monthly P&L dashboard view | ✅ Done — see dashboard.py |
...
- **Price forecasting remains the blocker for real trading — now started, not solved.** Phase 1 (replay.py) and Phase 2 (shadow.py) both decide using published/already-known prices. `forecast.py` (v15) established measured baselines, but accuracy is not yet tradeable (see section 10b). Open questions from here:
  ...
  - **✅ ADDRESSED (v26/v27) — CVaR-hedged stochastic DA optimisation.** See section 10e for the full result. A genuine, proven hedging mechanism (not a disguised point-forecast, per the mean-equivalence proof), built on empirical historical forecast-error scenarios rather than an assumed noise shape. A 58-day pilot looked promising; the full 718-day re-run showed that was small-sample noise — at the tested settings (λ=0.25, α=0.90) the hedge costs more than it protects, and even makes the single worst day slightly worse, not better. Not adopted anywhere. A broader λ/α sweep at full scale (not just the 58-day one this pilot used) is the natural follow-up if this is revisited, not a re-design of the mechanism itself.
- Stochastic optimisation — price uncertainty modelling approaches — ✅ built and piloted (v26, DA-only); full multi-stage DA→ID→BM recourse remains unexplored
...
- **v25 — shadow.py's DA leg now trades on a real forecast, closing a gap the project has been honest about since Phase 1.** Until today, `shadow.py` reused `dispatcher.py`'s DA logic unmodified from `replay.py` — meaning it built its schedule using the *real, already-known* day-ahead price, exactly like the Phase 1 perfect-foresight replay, just run one day at a time. That's not what a real trader can do: DA bids must be committed before the real clearing price is known. `forecast_pnl.py` already proved `reg_demand` captures ~87% of the DA-layer ceiling when trading blind — this wires that same method into the actual daily log for the first time.
  **What changed, precisely:** `dispatcher.py::run_dispatcher()` gained an opt-in `da_forecast_method` parameter (default `None`, preserving the exact original behaviour — decide AND settle on the real price — so **`replay.py`'s Phase 1 results are byte-identical, verified**: re-ran 2026-09-14 with no forecast method and reproduced the already-logged £69,465.36 net P&L exactly). When a method is requested, the DA leg's charge/discharge *decisions* are built on that forecast, but a new `settle_price` column (always the real price) is what actually gets paid — settling against your own forecast would silently overstate performance, the same mistake `forecast_pnl.py`'s `settle()` function was built to avoid. `shadow.py` now passes `da_forecast_method="reg_demand"` by default, with an automatic fallback to `mean_7` if `reg_demand`'s wind/solar/demand inputs are unavailable for a date, and to the real price only if neither forecast can be computed at all (i.e., too early in the dataset's history to have any prior days).
  **ID and BM are deliberately untouched.** ID's synthetic price model is still derived from the real DA price regardless of `da_basis` — it isn't a forecast-quality question, matching `forecast_pnl.py`'s own explicit DA-only scoping — and BM already settled on real SSP/SBP. Verified: re-running 2026-09-14 with the forecast enabled left `id_schedule_2026-09-14.csv` and `bm_schedule_2026-09-14.csv` byte-identical to the real-price run.
  **The 91 days already in `shadow_pnl.csv` were NOT retroactively recomputed.** They used the real-price methodology honestly, at the time, and rewriting history to look more sophisticated than it was would be exactly the kind of thing this project's own practice argues against. A new `da_basis` column (backfilled `"real"` for all 91 existing rows, `"reg_demand"` going forward) makes the log self-describing, so anyone reading it later knows which methodology produced which row without guessing.
  **First real test:** ran the actual `shadow.py` code path (not just `dispatcher.py` directly) against 2026-09-14 into a scratch log — `reg_demand` basis, net P&L £67,234.37, versus £69,465.36 under the real-price basis for the same date. A ~3.2% haircut from trading on forecast instead of hindsight — smaller than DA-only capture might suggest, since ID/BM (unaffected by the forecast) make up part of the day's total P&L. Consistent with the whole project's finding that forecast quality captures most, not all, of the theoretical ceiling.

- **v26 — CVaR-hedged stochastic DA optimisation, built and pilot-tested, genuine but mixed result.** Full detail in section 10e; summarised here for the changelog record.
  **The mathematical check that came first, before any code was trusted:** a plain "maximise expected revenue across scenarios" stochastic LP is provably identical to solving the existing deterministic LP once on the scenario mean price (shared decisions across scenarios + linear objective + no scenario-dependent constraints ⇒ linearity of expectation collapses it). Confirmed numerically, not just algebraically: `optimiser_lp_stochastic.py`'s own smoke test showed λ=0 matching the deterministic LP to **£0.0000** difference on real scenario data, and the same equality re-confirmed independently inside the later 20-day sweep. Without CVaR (Conditional Value at Risk — the standard Rockafellar-Uryasev linear formulation), this feature would have been a disguised, weaker version of "use a better point forecast," not real hedging.
  **New empirical foundation, not an assumed noise shape:** `forecast_residuals.py` reconstructed `reg_demand`'s actual historical forecast error, period by period, across 722 days — nothing in the codebase had stored this before. Spot-checked against the independently-known MAE (£17.73 vs £17.72/MWh — matched). Scenarios bootstrap *whole real historical days'* error patterns onto today's forecast, preserving realistic within-day correlation rather than assuming independent per-period noise like `simulate_intraday_prices` does for ID.
  **A real methodological bug caught and fixed during scoping, not shipped silently:** the first version of the scenario sampler used `fill_value=None` when combining a borrowed day's residual with today's forecast, which silently dropped whichever periods that specific historical day happened to be short on — different scenarios in the same list ended up with different shapes, which would have crashed or silently misaligned the CVaR LP. Fixed to `fill_value=0` (a missing residual means "no error information for that period," not "drop it"), verified by re-running the exact case that had failed.
  **A second real issue caught while testing, not assumed away:** a genuine £1,353/MWh price spike on 2025-01-08 (the all-time high in the dataset, a real cold-snap event the forecast completely missed) produced a residual so large that transplanting it onto an unrelated calm day produced a scenario price of roughly -£991/MWh — never observed, never possible on that day. Clipped scenarios to [-£200, £1,500] (wider than the real observed [-£103, £1,353] range) to preserve genuine tail risk while ruling out this kind of combination artifact.
  **A third issue caught before trusting the λ-sweep:** the first sweep run drew a fresh random scenario set on every separate invocation, so `ensemble_mean` (which cannot depend on λ at all) visibly drifted between λ values — caught by noticing numbers that were structurally required to be identical weren't. Fixed by seeding scenario generation deterministically per target date, confirmed by re-running the sweep and seeing `deterministic`/`ensemble_mean` become perfectly stable across every λ.
  **Wired into `dispatcher.py`** via a `"_stochastic"` suffix on `da_forecast_method`, only engaging when the underlying point forecast succeeded with no fallback. Verified non-regression on 2026-09-14: `None` and `"reg_demand"` reproduced their already-known exact figures; `"reg_demand_stochastic"` ran cleanly and left `id_schedule`/`bm_schedule` byte-identical (same SHA-256) to the non-stochastic run.
  **The pilot result (58 real days, cost-aware, α=0.90, λ=0.25 chosen from a 20-day sweep):** a better central estimate (averaging multiple forecast methods) did NOT help (-£444/day vs the plain point forecast — a real negative finding). The CVaR hedge DID genuinely improve the worst-10%-of-days average (+£1,755/day vs the ensemble mean), exactly as designed, but at a larger average-day cost (-£637/day vs the plain deterministic arm) — an insurance premium that outweighed its payout in raw pounds on this sample. The single worst day barely moved under any setting, suggesting a DA-only hedge can't fix whatever makes that specific day bad.
  **What did NOT change, explicitly checked:** `optimiser_lp.py::optimise_battery_lp` (zero edits), `forecast.py` (zero edits), `replay.py` (zero edits, unaffected by construction), `dispatcher.py`'s non-stochastic behaviour (verified byte-identical), `shadow.py`'s default (`reg_demand`, still non-stochastic — deliberately not switched based on a mixed 58-day pilot).
  **Not yet done, flagged rather than silently deferred:** a larger-sample pilot (the full ~770-day history, not just 58 days — this project has been burned by small samples before, e.g. the v16 `weekday` illusion), a proper λ/α sensitivity sweep beyond the coarse 4-point grid used here, and full multi-stage DA→ID→BM recourse (this pass hedges DA only, by explicit scope decision, not as a limitation discovered after the fact).

- **v27 — the CVaR pilot's tail-improvement finding was small-sample noise; re-run at full scale (718 real days) reverses the "mixed" read into a real answer.** Directly closes the v26 "not yet done" item above. Same script (`da_stochastic_pnl.py`), same λ=0.25/α=0.90 chosen from v26's sweep, run against all available history (`python3 da_stochastic_pnl.py 800 0.25`) instead of the last 58 days — 12.4 minutes, confirming the earlier 1.0s/day timing held at scale.
  **What changed between 58 days and 718 days:** the 58-day pilot showed the hedge improving the worst-10%-of-days average (p5) by +£1,755/day for a £193/day average cost — genuinely promising-looking. At full scale, the tail improvement shrinks to **+£466/day** while the average cost *grows* to **£306/day** — the insurance premium is now clearly larger than the payout, not just arguably so. Worse: the single worst day, which the pilot suggested was roughly unaffected, is actually **£731 more negative** under the hedge at full scale (−£13,449 → −£14,180) — the hedge making the worst case *worse*, the opposite of its purpose. Capture declines monotonically across all three arms (86.6% → 85.2% → 84.3%) with no offsetting benefit anywhere.
  **The central-estimate finding held up unchanged**: −£438/day at full scale vs −£444/day on the pilot — averaging forecast methods together doesn't help, confirmed rather than contradicted by more data.
  **Verified reproducible, not just re-run**: the original 58 days' rows in `data/da_stochastic_pnl.csv` came back byte-identical inside the 718-day run (pure append in the diff, zero rows modified) — confirms the per-date deterministic scenario seeding from v26 holds under a different run context, not just the one it was fixed in.
  **Conclusion, revised honestly rather than left ambiguous:** at λ=0.25/α=0.90, this hedge does not pay for itself. The mechanism (empirical scenarios, the CVaR LP, the `dispatcher.py` wiring) is unaffected and still sound — this result is about the parameter choice and payoff at this scale, not a flaw in the approach. Whether a different λ/α would behave differently is untested at full scale and is the natural next step only if this is revisited; it is not assumed to rescue the current result. `shadow.py`'s default remains `reg_demand` (non-stochastic).

- **v28 — the live dispatch/shadow path was cost-blind for its entire history, despite v20's cost-awareness being built and proven.** Found while scoping the next algorithm-improvement task (joint DA/ID/BM optimisation): the user asked directly whether the charge/discharge algorithm had actually been improved. Checking the answer honestly surfaced that `dispatcher.py`'s live DA calls never passed `cost_discharge`/`cost_charge` to `optimise_battery_lp`/`optimise_battery_lp_cvar`, silently defaulting to `0.0`, and `shadow.py`'s settlement math (`gross()`) never applied execution costs either — a raw `power_mw × price` calc with no cost term anywhere. **Every one of the 93 days in `shadow_pnl.csv`, including every `reg_demand` (v25) row, was computed completely cost-blind** — the proven cost-aware improvement from v20 only ever ran inside the standalone research scripts (`forecast_pnl.py`, `cost_sensitivity.py`), never in the system actually producing the daily log.
  **Fix:** `dispatcher.py`'s DA leg now passes `cost_discharge = COST_IMPACT + COST_FEE + COST_DEGRADATION`, `cost_charge = COST_IMPACT + COST_FEE` into both the deterministic and CVaR-stochastic optimiser calls — the exact expression already used in `forecast_pnl.py`/`da_stochastic_pnl.py`'s `COST_KW`, reused rather than re-derived. Settlement gained two new cost-adjusted columns (`settle_price_discharge`, `settle_price_charge`), mirroring `forecast_pnl.py::settle(..., with_costs=True)`'s asymmetric adjustment (discharge paid price-minus-costs, charge costs price-plus-costs) rather than the old single `settle_price` column, which was real but still cost-blind. `shadow.py` now settles on these instead, so `net_pnl` reflects real execution economics for the first time.
  **Verified against an already-established number, not a fresh assumption:** re-ran 2026-09-14 (real basis) — DA-leg-only net fell from £46,341.67 to £43,941.06, a **5.18%** reduction, the same order of magnitude as v20's independently-measured ~8.3% DA-only figure (a single day vs. a 681-day average, so not identical, but consistent). Full-portfolio net P&L fell from £69,465.36 to £67,064.75 (~3.5%, smaller because ID/BM are untouched and dilute the DA-only effect). **ID/BM schedule files confirmed byte-identical (same SHA-256) before and after** — the DA-only scope boundary held, same check used for every prior change this session. Ran `shadow.py`'s actual code path (not just `dispatcher.py`) against 2026-09-15 into a scratch log: `cost_aware=True`, `da_net` fell from £53,205.29 to £47,644.29, `net_pnl` from £107,346.46 to £101,785.47 — the mechanism works end to end.
  **The 93 days already in `shadow_pnl.csv` were NOT retroactively recomputed** — same honesty policy as `da_basis` in v25. A new `cost_aware` column (backfilled `False` for all 93 existing rows, `True` going forward) makes the log self-describing about which rows reflect real costs.
  **What remains cost-blind, explicitly flagged rather than implied fixed:** `optimiser_id.py`/`optimiser_bm.py` have **no cost parameters at all** — not a wiring gap like this one, a genuine missing feature that was never built or tested for ID/BM. This fix closes the DA gap only; ID and BM P&L in `shadow_pnl.csv` still doesn't reflect real execution costs. `replay.py` (Phase 1) is deliberately untouched — it's the explicitly-labelled perfect-foresight ceiling, never presented as a trading result regardless of cost treatment.

- **v29 — ID and BM closed the cost-awareness gap v28 explicitly left open; the live path has no remaining cost-blind leg.** `optimise_battery_id`/`optimise_battery_bm` gained the same `cost_discharge`/`cost_charge` parameters as `optimise_battery_lp` (default `0.0`, so any other caller is unaffected), applied inside each LP objective the same way. `dispatcher.py` passes the **same** cost figures already computed for the DA leg into both — degradation, exchange fees, and market impact are properties of trading the physical battery, not of which market layer the trade happens in, so reusing one set of constants across all three legs is a deliberate simplification (documented here, same spirit as the DA cost model). Settlement gained matching cost-adjusted columns (`id_price_discharge`/`id_price_charge`, `ssp_settle`/`sbp_settle`), and `shadow.py` now settles ID/BM on these instead of raw price × power. BM's bespoke inline revenue/cost calc was also folded into the shared `gross_rev_cost` helper `dispatcher.py` already used for DA/ID, removing duplication.
  **Verified against 2026-09-14 (same regression date as v28):** full-portfolio net P&L fell from £67,064.75 (v28, DA-only cost-aware) to £64,169.65 — a further **4.3%** reduction, smaller than DA's own effect as expected since ID/BM each carry a smaller committed capacity share (20-30% vs DA's 40-50%). ID's schedule changed materially (110→125 hold periods — more thin spreads now correctly skipped); BM's schedule was unchanged on this particular day (its SSP/SBP spread was wide enough that the flat cost didn't tip any period), but its settlement is still cost-adjusted, so `bm_net` still fell. Ran `shadow.py`'s real code path against 2026-09-15 into a scratch log: `cost_aware=True`, `net_pnl` fell further from v28's £101,785.47 to **£98,661.35**.
  **One genuine exception surfaced by timing, not by this fix's own design:** at the point this work started, `shadow_pnl.csv` had 93 rows, all `cost_aware=False`, and zero `True` — the v28 fix had only ever been tested against scratch paths, never the live file, so no backfill/relabeling was expected to be needed. Mid-fix, the daily pipeline ran unattended and shadow-logged **2026-09-16** using the not-yet-merged v28 code, producing the first-ever `cost_aware=True` row — but one that only reflects v28's DA-only cost-awareness, since v29 wasn't live yet. **This row is not relabelled or rewritten** — same "never retroactively rewrite logged history" policy as `da_basis` in v25 and `cost_aware` itself in v28 — but it means `cost_aware=True` has one transitional exception: 2026-09-16 is DA-only cost-aware, not full DA+ID+BM like every `True` row from 2026-09-17 onward. Flagged in `dashboard.py`'s caption so this doesn't silently understate what "cost-aware" means for that one day.
  **Also noticed while investigating this, unrelated and out of scope:** the same pipeline run also produced an exploration note (`data/explorations/2026-09-17/`) comparing the first live cost-aware dispatch day against price volatility — worth a look separately, not reviewed as part of this fix. (Reviewed afterwards: not a bug — the 2026-09-16 P&L drop is confounded by a calm-price day, so it is a watch item, not a measured effect size.)

- **v30 — scoping "joint DA/ID/BM optimisation" and sweeping the capacity split surfaced a much bigger issue: the BM leg is a perfect-foresight leg, and ~29% of the logged shadow P&L rests on it.** Two findings, in order of discovery:
  **(1) A naive joint LP would be a look-ahead trap.** The simulated ID price is `real DA price + noise` (`optimiser_id.py`, fed the real `da_prices` in `dispatcher.py`), so merging the three LPs would feed real ID/BM data into a DA decision that must be committed before it exists. The current sequential DA→ID→BM design is realistic in its *ordering* (DA on forecast; ID after DA clears, so knowing the DA price is legitimate; BM last). A proper joint model would need a multi-stage stochastic program with ID/BM scenario generation — a CVaR-sized build with, given v27, uncertain payoff — so it was deliberately not pursued. Instead the never-tuned fixed capacity split (`config.py`: 2-hour Battery_1 40/30/30, 4-hour Batteries 2-5 50/20/30, no documented rationale) was swept.
  **(2) The sweep result is an artifact, not an opportunity.** `reservation_sensitivity.py` (new; subprocess-per-scenario via new `VPP_RES_ID_4H/BM_4H/ID_2H/BM_2H` env overrides in `config.py`, mirroring `cost_sensitivity.py`) ran 36 (DA/ID/BM) splits for the 4-hour class over the last 60 days on the live methodology (`reg_demand` DA, cost-aware). P&L rose almost monotonically as capacity moved from DA into BM: best DA=20/ID=20/BM=60 at £5.10M vs the current default's £4.56M (+11.9%). Before validating at full scale, the reason was checked — and it is structural, so more data would only have confirmed the same artifact. **`system_prices` SSP == SBP on all 775 cached days (the single GB imbalance price, not a BM bid/offer acceptance price), and `optimiser_bm.py` is handed the entire day's real imbalance-price series as its decision input** — perfect foresight on the most volatile series in the model, with no fill/acceptance constraint and no own-position price impact. Re-deciding BM on a naive forecast and settling at the real price (60 days, default split, cost-aware, each leg from 50% SOC; scratch test): **foresight £52.9k/day → mean-of-last-7-days £33.5k (−37%) → yesterday's prices £27.1k (−49%)**. So the optimiser "found" that the leg with the most foresight per MW is worth more MW.
  **ID is largely fine — this corrects an initial suspicion.** Knowing the iid noise draw is worth ~0.4% of ID P&L (£37.5k vs £37.3k/day when decided on the DA price alone); ID's P&L comes from the DA price shape, and knowing the real DA price at ID time is legitimate. Its real caveat is the previously documented one: the ID price itself is synthetic.
  **What this means for reported numbers.** Over the 94-day shadow log, average net P&L is £82.7k/day: DA £49.1k (59%), ID £9.8k (12%), BM £23.8k (29%). The DA leg is forecast-driven and defensible; **the BM share is perfect-foresight and the ID share rests on a synthetic price — the logged *total* is not investor-safe as a trading estimate, and section 10d's DA-only, forecast-driven, cost-aware figures remain the defensible ones.** Any external use of `shadow_pnl.csv` totals or the dashboard's Monthly P&L headline must carry this caveat until BM is decided on a forecast. No hedged/blind figure exists yet for the full stack.
  **Not adopted, nothing changed in live behaviour:** the swept splits are NOT proposed for `config.py`; live defaults are unchanged (regression: 2026-09-14 still £64,169.65 after the `config.py` refactor). `config.py` now derives `DA_RESERVATION` as `1 − ID − BM` (values identical to before; fixes the previously-dead DA_RESERVATION, which `dispatcher.py` never read). `dispatcher.py` gained `write_schedules=True` so sweeps can run without overwriting the tracked schedule CSVs (the first pilot did overwrite ~180 of them; restored from git before committing). Pilot rows are kept in `data/reservation_sensitivity_v30_foresight_bm.csv` (renamed in v31 to mark that they were produced with foresight BM) as the honest record — read them as evidence of the BM artifact, not as a tuning result.
  **Next step this pointed to (done in v31):** put BM on the same footing as DA — decide on a forecast, settle at the real imbalance price (v25's `da_basis` pattern, e.g. a `bm_basis` label with history left as logged), then re-run the split sweep, which only becomes meaningful once BM no longer has foresight. Real BM also has no guaranteed fill (NESO chooses acceptances; pay-as-bid), which is a separate, larger realism gap.
  **Also noticed, unrelated:** `models/pnl.py` reads a `da_schedule_{date}.csv` the pipeline no longer produces (orphaned).

- **v31 — the BM leg no longer decides with perfect foresight; the shadow log's BM/total figures are now honestly labelled and (going forward) realistic on this axis.** Closes the gap v30 found. **Design chosen by a bake-off, not assumed:** scratch test over 200 days (2026-03-05..09-20), BM leg only, 30% capacity, cost-aware, settled at the real imbalance price, comparing what BM could *legitimately* decide on — perfect foresight (ceiling) £43.9k/day; **real DA price £36.9k (84.1% of ceiling, no losing days)**; real DA + 7-day mean imbalance-minus-DA spread £35.7k (81.3%); 50/50 blend £33.8k (77.0%); 7-day mean of imbalance prices £27.2k (62.1%); yesterday's imbalance prices £21.6k (49.3%, 6 losing days). The simplest option won and the fancier ones did worse. It is also legitimate: DA has cleared before BM matters, the same reason ID may know the DA price.
  **Change:** `optimiser_bm.py` gained `decision_prices=None` (LP decides on it; returned `ssp`/`sbp` and settlement stay real; `None` is byte-identical to before). `run_dispatcher()` gained `bm_decision_method` (`None` = foresight, the default — so `replay.py`, the labelled perfect-foresight ceiling, is untouched; `"real_da"` = decide on the real DA price reindexed to BM's periods, ffill/bfill for clock-change days) and records `bm_basis` in `df_lp.attrs` next to `da_basis`. `shadow.py` now passes `"real_da"` and logs a new `bm_basis` column; all 97 existing rows were backfilled `"foresight"` (label, not rewrite — same precedent as `da_basis` v25 and `cost_aware` v28). `reservation_sensitivity.py`'s worker also decides BM on `"real_da"`.
  **Verified:** default path byte-identical (2026-09-14: £64,169.65, SHA-256 of all three schedule files unchanged). With `"real_da"` the DA and ID schedules are identical and only BM changes. Full chain with SOC handoff over the last 60 days (reg_demand DA, cost-aware): **BM £24.0k → £16.9k/day (−29.6%), total £79.0k → £71.9k/day (−9.0%)**, DA and ID exactly unchanged — that ~9% is the overstatement the foresight leg was adding to every pre-v31 logged total. (The full-chain BM drop is larger than the standalone bake-off's −16% because in the chain BM starts from a nearly empty battery after DA/ID.) `shadow.py`'s real path into a scratch log for 2026-09-20: `bm_basis=real_da`, DA/ID unchanged, `bm_net` £63.6k → £62.5k (a day where DA and imbalance prices moved together, so foresight added little). A new-schema append onto the backfilled log lands in the right columns.
  **What is and isn't fixed.** Fixed: BM foresight. **Still not modelled, so BM remains an imbalance-exposure figure rather than a defensible standalone revenue claim:** BM fill/acceptance (NESO chooses what is taken; pay-as-bid), and own-position price impact on the imbalance price. **ID's noise foresight** (~0.4% of ID P&L) is documented, not fixed; ID also still trades a synthetic price. Pre-v31 rows (`bm_basis=foresight`) overstate P&L and should not be compared with later rows without that in mind; the dashboard now shows a `bm_basis` breakdown and an updated caveat.
  **Split sweep re-run on honest BM (4-hour class; `data/reservation_sensitivity.csv`, v30's foresight-BM run is `..._v30_foresight_bm.csv`).** *Pilot, 60 days, 36 splits:* the v30 artifact is gone — BM's optimum moved from 60-70% to 40-50% and the best split's edge over the default (DA50/ID20/BM30, £4.31M) shrank from +11.9% to **+5.5%** (DA20/ID40/BM40, £4.55M); the top five splits sit within ~1.5% of each other, so the pilot cannot rank them. *Full-history validation, 650 days, same days for both arms:* default £29.19M (£44.9k/day) vs DA20/ID40/BM40 £31.30M (£48.2k/day), **+7.2%** — it strengthened rather than reversing, unlike CVaR in v27.
  **The forecast-error hypothesis was tested directly, not left as speculation, and the picture is more precise than "artifact."** `optimiser_id.py` gained the same `decision_prices` override as `optimiser_bm.py` (v31); `dispatcher.py` gained `id_decision_method`/`bm_decision_method="da_forecast"` (diagnostic only, never used by `shadow.py`) that decides ID *and* BM on the same DA forecast the DA leg itself trades on, instead of the real DA price — putting every leg on equal "has to guess" footing. Re-running the sweep this way (`reservation_sensitivity.py pilot/validate ... blind`, vs the realistic mode's default) isolates two components of the +7.2%:
  - **Pilot, 60 days, blind mode:** best split's edge over default shrinks from +5.5% (realistic) to **+1.5%** (DA40/ID20/BM40 — only a 10-point DA trim, not the realistic mode's 30-point one). **Full-history validation, 650 days:** default £38,534/day vs candidate £39,079/day, **+1.4%**, holds without reversing.
  - So roughly **1.4 of the 7.2 points is a small, real edge that survives even with no informational advantage between legs. The other ~5.8 points comes specifically from ID/BM correcting DA's forecast error** (`reg_demand` captures ~87% of the DA-shape ceiling; deciding after the real DA price is known effectively captures the rest) — a real, legitimate mechanism (ID/BM genuinely do know the real DA price by the time they trade, matching real market timing; this is not a foresight cheat like v30's BM bug), but one that is contingent on current forecast quality, not a fixed structural fact about these markets. If DA forecast accuracy improves, this component should shrink.
  **Still not adopted.** Even the durable-looking 1.4-point component sits on top of two unresolved realism gaps — ID's synthetic price (real DA + Normal(0, £5)) and BM's unmodelled fill/acceptance — so a capacity commitment based on it would concentrate the book in the least-realistically-modelled legs. `config.py` defaults are unchanged. **Revisit when ID has a realistic price model or BM acceptance is modelled**, and re-check this decomposition periodically as DA forecast accuracy work continues — the larger component is explicitly expected to erode, not a one-time finding. Only the 4-hour class was swept; Battery_1 (2-hour) was not.

- **v32 — quantified BM's fill/acceptance gap; smaller than expected, and not what's inflating reported BM P&L.** `system_prices_{date}.csv` (already fetched from Elexon's settlement endpoint) has always carried `totalAcceptedOfferVolume`/`totalAcceptedBidVolume` - the real GB-wide volume NESO actually accepted each period, the same figures Elexon uses to derive SSP/SBP. Nothing in the codebase read them. New `models/bm_fill_diagnostic.py` compares the portfolio's modelled BM trade volume each period against this real system-wide figure, without inventing a specific capture-rate assumption (there's no way to derive "our addressable share" from a system-wide total, so none is assumed).
  **Result, 124 days with both files available (2025-03-30 to 2026-09-21), 4,429 BM-active periods:** median ratio 2.4% of GB-wide accepted volume, 90th percentile 6.9%, 99th percentile 27.5%. **26 periods (0.6%) had essentially zero GB-wide accepted volume on that side yet the model traded anyway** - the starkest case, an outright-impossible fill at any capture rate. Including those, **5.9% of BM-active periods** would represent an illustrative >10%-of-the-whole-GB-market slice from a single 145 MW portfolio. **The share of logged BM P&L coming from those flagged periods is -1.3%** (i.e. slightly negative) - so unlike the v30 foresight bug, this gap is not what's inflating BM's ~29% share of logged P&L; it's a real but narrow tail issue (0.6% outright-impossible trades, ~6% implausibly large ones), not a broad one.
  **Not fixed, and the reason is data, not effort:** `totalAcceptedOfferVolume`/`BidVolume` are GB-wide totals with no way to derive a 145 MW portfolio's realistic addressable share from them - baking in a guessed capture-rate cap would trade one unvalidated assumption (infinite fill) for another (some guessed %), which is exactly the overclaiming this project avoids elsewhere. The planned follow-up (an illustrative capture-rate sensitivity sweep, `cost_sensitivity.py`-style) was **not built**, because the diagnostic already answered the question this session needed: the gap is real but modest, not a major driver of reported BM P&L. Revisit only if participant-level GB BM data becomes available, or if a future backtest shows this ratio growing (e.g. portfolio MW scaling up).
---
