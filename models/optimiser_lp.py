...
def optimise_battery_lp(battery, price_series, committed_capacity=1.0):
    ...
    DURATION = 0.5  # 30-minute settlement period in hours
    ...
    # --- Constraints ---
    # Energy balance with correct time factor:
    # Energy in = power (MW) × duration (h) × efficiency
    # Energy out = power (MW) × duration (h) / efficiency
    initial_soc = 0.50 * battery.capacity_mwh

    for i, t in enumerate(T):
        if i == 0:
            # First period: SOC = initial + charge_energy - discharge_energy
            prob += soc[t] == initial_soc + charge[t] * DURATION * battery.efficiency - discharge[t] * DURATION / battery.efficiency
        else:
            # Subsequent periods: SOC = previous SOC + charge_energy - discharge_energy
            prev_t = T[i-1]
